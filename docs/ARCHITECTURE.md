...
### 主な特徴
- **リアルタイムマルチプレイヤー**: 定期ポーリングによる準リアルタイム同期
- **ルームベース**: 6文字コードでの簡単参加
- **匿名投票システム**: 公平な評価システム
...
**Oogiri App** では、シンプルで信頼性の高い**定期ポーリング**を使用してリアルタイム更新を実現しています：
...
1. **実装の簡単さ**: 
   - 標準的なHTTP APIリクエストのみ使用
   - WebSocketやSSEの複雑な接続管理が不要
...
// ゲーム状態API レスポンス
{
  gameSession: {
    id: string,
    status: 'waiting' | 'question' | 'answering' | 'voting' | 'results',
    currentRound: number,
    totalRounds: number
  },
  players: Array<{
...
2. **バックエンド**: カスタムサーバー (server.js)
3. **データベース**: PostgreSQL + Drizzle ORM  
4. **認証**: NextAuth.js + Google OAuth
5. **リアルタイム**: 定期ポーリングによる準リアルタイム更新

**server.js** はアプリケーションの心臓部で、Next.jsアプリケーションを本番環境で安定動作させるためのカスタムサーバーです。
